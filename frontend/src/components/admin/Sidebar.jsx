...
const Sidebar = ({ isOpen, setIsOpen }) => {
    ...
    const menuItems = [
        ...
        },
        {
            ...
            label: 'Products',
            items: [
                { path: '/admin/products', icon: <FiList className="w-4 h-4" />, label: 'All Products' },
                { path: '/admin/add-product', icon: <FiPlus className="w-4 h-4" />, label: 'Add Product' },
            ]
        },
        {
            type: 'single',
    ...
    return (
        <>
                            ...
                            {item.type === 'single' ? (
                                ...
                                <div>
                                    <button
                                        ...
                                        className={`flex items-center justify-between w-full px-3 py-3 rounded-lg transition-all duration-200 group ${openDropdown === item.key || item.items.some(subItem => location.pathname === subItem.path)
                                            ? 'bg-blue-50 text-blue-600 border border-blue-200 font-medium'
                                            : 'text-gray-600 hover:bg-gray-50 hover:text-gray-900'
                                            }`}
                                    >
                                        ...
                                        <div className="ml-6 mt-1 space-y-1 border-l-2 border-gray-100 pl-3 py-1">
                                            {item.items.map((subItem, subIndex) => (
                                                <NavLink
                                                    ...
                                                    to={subItem.path}
                                                    className={({ isActive }) =>
                                                        `flex items-center gap-2 px-3 py-2 rounded-lg text-sm transition-all duration-200 group ${isActive
            ...
            <div className={`fixed inset-0 z-50 lg:hidden transition-opacity duration-300 ${isOpen ? 'opacity-100 visible' : 'opacity-0 invisible'
                ...
                <div className={`absolute top-0 right-0 h-full w-80 bg-white shadow-xl transform transition-transform duration-300 ease-in-out ${isOpen ? 'translate-x-0' : 'translate-x-full'
                    }`}>
                            ...
                            <div key={item.key || item.path}>
                                {item.type === 'single' ? (
                                    <NavLink
                                        ...
                                        onClick={() => setIsOpen(false)}
                                        className={({ isActive }) =>
                                            `flex items-center gap-4 px-4 py-4 rounded-xl transition-all duration-200 group ${isActive
                                    ...
                                    <div>
                                        <button
                                            onClick={() => toggleDropdown(item.key)}
                                            className={`flex items-center justify-between w-full px-4 py-4 rounded-xl transition-all duration-200 group ${openDropdown === item.key || item.items.some(subItem => location.pathname === subItem.path)
                                                ? 'bg-blue-50 text-blue-600 border-2 border-blue-200 font-semibold'
                                                : 'text-gray-700 hover:bg-gray-50 hover:text-gray-900'
                                                }`}
                                        >
                                            ...
                                            <div className="ml-8 mt-2 space-y-2 border-l-2 border-gray-200 pl-4 py-2">
                                                {item.items.map((subItem, subIndex) => (
                                                    <NavLink
                                                        ...
                                                        onClick={() => setIsOpen(false)}
                                                        className={({ isActive }) =>
                                                            `flex items-center gap-3 px-4 py-3 rounded-lg text-[15px] transition-all duration-200 group ${isActive
